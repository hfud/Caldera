...
class DebriefService(BaseService):
    ...
    @staticmethod
    def generate_ttps_nested(operations):
        ttps = dict()
        for op in operations:
            for link in op.chain:
                if link.cleanup:
                ...
                if tactic_name not in ttps:
                    ttps[tactic_name] = {
                        'name': tactic_name,
                        'techniques': dict()
                    }

                techniques = ttps[tactic_name]['techniques']
                ...
                abilities = techniques[technique_name]['abilities']
                if op.name not in abilities:
                    abilities[op.name] = []

                if ability_name not in abilities[op.name]:
                    abilities[op.name].append(ability_name)
